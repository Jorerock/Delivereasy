...
const tournee : Tournee =  {Tournee_ID:1,Utilisateur_ID:1};


const Tableau = ref<any[]>([]);

onMounted(async () => {
  const livraisonsRequest = await fetch('http://localhost:3000/tournee/all', {
    method: 'GET',
    credentials: 'include',
    headers: {
...
// Modif livraison
const onlivraisonInput = async (newtourneeValue: Tournee, index: number) => {
  ...
  await fetch(`http://localhost:3000/tournee/`, {
    ...
    body: JSON.stringify({
      Tournee_ID: newtourneeValue.Tournee_ID,
      Utilisateur_ID: newtourneeValue.Utilisateur_ID,

    }),

  });
  console.log( newtourneeValue.Tournee_ID +'a ete mis à jour et la modification est envoyée au serveur');
};
...
<template>
  ...
  <!-- <table> -->
    ...
    <tourneeComponent :cli="element" @onInput="onlivraisonInput($event, index)" />
